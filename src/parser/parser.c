...
Scope* new_scope(Declaration* parent) {
    ...
    Scope* boxed_scope = (void*)new_node((Node)
        {
            .
            Scope = scope
        }
...
Wrapper* get(Scope scope, Trace identifier) {
    ...
    ScopeEntries entries = scope.data[fnv1a_u32_hash(identifier.source) % scope.cap];
    for (size_t i = 0; i < entries.size; i++)
    {
        if (streq(identifier.source, entries.data[i].identifier))
        {
