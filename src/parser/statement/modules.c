...
Parser find_import(String relative_path, str identifier, Trace trace, Parser* parser) {
    ...
    for(size_t i = 0; i < len(global_library_paths); i++) {
        ...
        if(!input_content) {
            strf(&path_collection, i ? ", %.*s" : "%.*s", fmtof(import_path));
            len(import_path) = 0;
            continue;
        }
    ...
    push(parser->tokenizer->messages, MERROR(trace, strf(0, "unable to open or read %s", path_collection)));
    return (Parser) { 0 };
}
