...
#define STR_FMT "%.*s"
#define STR_ARGS(str__) (int)str__.size, str__.data
...
int print_message(Message message) {
    ...
    {
        // TODO: see #1
        printf("\33[1m%s%s:\33[0m "STR_FMT"\n",
               message.highlight, message.label,
               STR_ARGS(message.content));
        return 0;
    }
    ...
    // TODO: see #1
    printf("\33[1m%s:%u:%u: %s%s:\33[0m "STR_FMT"\n"
           "%4d | %s\n     : %s%.*s\33[0m\n",
           message.trace.filename, message.trace.row, message.trace.col,
           message.highlight, message.label, STR_ARGS(message.content),
